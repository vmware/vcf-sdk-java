...
/**
 * Section 3.2: The Version Attribute MUST be present and MUST contain the value "2.0", the ID attribute MUST be preent
 * and MUST contain a UUID with a "_" prefix and the IssueInstant MUST be in UTC time.
 *
 * <p>An assertion MUST contain an AuthnStatement but cannot contain an AuthZDecisionStatement or a Statement.
 *
 * <p>TODO: The current schema is not quite correct since it requires the AuthnStatement to be before the
 * AttributeStatement.
 *
 * <p>SAML 2.0: An assertion with no statements MUST contain a Subject element. Such an assertion identifies a principal
 * in a manner which can be referenced or confirmed using SAML methods, but asserts no further information associated
 * with that principal.
 *
...
@XmlType(
        name = "AssertionType",
        propOrder = {"issuer", "signature", "subject", "conditions", "advice", "authnStatementOrAttributeStatement"})
public class AssertionType {
     ...
     * for the authnStatementOrAttributeStatement property.
     *
     ...
     * </pre>
     *
     * <p>Objects of the following type(s) are allowed in the list {@link AuthnStatementType }
