...
public final class Util {
    ...
    static {
        ...
        disableTransformerFactoryAttribute(XMLConstants.ACCESS_EXTERNAL_DTD);
        disableTransformerFactoryAttribute(XMLConstants.ACCESS_EXTERNAL_STYLESHEET);
    }

    private static void disableTransformerFactoryAttribute(String attribute) {
        try {
            TRANSFORMER_FACTORY.setAttribute(attribute, "");
        } catch (IllegalArgumentException e) {
            getLog().warn("{} does not support disabling {}", TRANSFORMER_FACTORY.getClass(), attribute, e);
        }
    }
     ...
     *
     * <p>The XML declaration {@literal <?xml ... >} is omitted for easier embedding/serialization.
     *
     * @param content, required
     * @return XML as string without XML declaration
     * @throws ParserException if serialization fails
     */
     ...
     *
     * @param token required, must be validated
     * @return description, not null TODO migrate this code into samltoken implementation
     */
    public static String createRedactedDescription(SamlToken token) {
        ...
        return String.format(
                "%s [subject=%s, groups=%s, delegationChain=%s, startTime=%s, "
                        + "expirationTime=%s, renewable=%s, delegable=%s, isSolution=%s,"
                        + "confirmationType=%s]",
                token.getClass().getSimpleName(),
                getTokenSubjectForLog(token),
                token.getGroupList(),
                token.getDelegationChain(),
                token.getStartTime(),
     ...
     *
     * <p>This function attempts to load an XML schema file (and any additional referenced schema files) from the
     * specified resource location, using the provided class loader.
     *
     * @param theClassLoader the {@link ClassLoader} used to locate and load the resource
     * @param resourceLocation the location or path to the resource
     * @param resourceName the name of the resource containing the XML schema
     * @return a {@link Schema} object representing the loaded XML schema
     * @throws IllegalArgumentException if the resource cannot be accessed, found, or read
     * @throws SAXException if an error occurs during XML schema parsing
     */
    public static Schema loadXmlSchemaFromResource(
            ClassLoader theClassLoader, String resourceLocation, String resourceName)
        ...
        try {
            getLog().debug(
                            "loadXmlSchemaFromResource([{}], [{}], [{}])",
                            new Object[] {theClassLoader.getClass().getName(), resourceLocation, resourceName});
     ...
     * Supports internal com.vmware.identity infrastructure. Not to be consumed by external clients.
     *
     * <p>This function attempts to load an XML schema file (and any additional referenced schema files) from the same
     * JAR where the specified class is located. The schema file should be accessible as a resource in the same JAR.
     *
     * @param theClass the reference {@link Class} used to locate the resource
     * @param resourceName the name of the resource containing the XML schema
     * @return a {@link Schema} object representing the loaded XML schema
     * @throws IllegalArgumentException if the resource cannot be accessed, found, or read
     * @throws SAXException if an error occurs during XML schema parsing
    ...
    public static Schema loadXmlSchemaFromResource(Class<?> theClass, String resourceName)
        ...
        //
        // implementation is an attempt to get away from some URLConnection/URLJarFile caching, and premature zip file
        // closures which result in random schema loading errors (refer bugzilla#988041, 1032736);
        try {
    ...
    private static Schema loadXmlSchemaFromResource(IResourceProvider resourceProvider, String resourceName)
        ...
        getLog().debug(
                        "Resources's [{}] InputStream type is: [{};{}]",
                        new Object[] {resourceName, is.getClass().getName(), is.toString()});
    ...
    private static class ClassLoaderResourceProvider implements IResourceProvider {
        ...
        public ClassLoaderResourceProvider(ClassLoader theClassLoader, String resourceLocation) {
            ...
            this._streams = new ArrayList<>(10);
            this._resourceLocation = (((resourceLocation == null) || (resourceLocation.isEmpty()))
                    ? ""
                    : ((resourceLocation.endsWith("/") ? resourceLocation : resourceLocation + "/")));
        }
    ...
    private static class ClassResourceProvider implements IResourceProvider {
        ...
        public ClassResourceProvider(Class<?> theClass, String mainResourceName) throws IOException {
            ...
            if (("jar".equalsIgnoreCase(url.getProtocol()))
                    && (url.getPath() != null)
                    && (url.getPath().toLowerCase(Locale.ENGLISH).startsWith(FILE_SCHEME_PREFIX))) {
    ...
    private static class ResourceResolver implements LSResourceResolver {
        ...
        public LSInput resolveResource(
            ...
            if (resourceAsStream == null) {
                ...
                getLog().debug("Resources's [{}] InputStream type is: [{};{}]", new Object[] {
                    systemId, resourceAsStream.getClass().getName(), resourceAsStream.toString()
                });
            }
