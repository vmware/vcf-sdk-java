...
/**
 * Demonstrates getting protection group snapshot.
 *
 * <p>Sample Prerequisites: vCenter 8.0.3+
 */
public class GetPGSnapshot {
    private static final Logger log = LoggerFactory.getLogger(GetPGSnapshot.class);
    ...
    /** REQUIRED: Snapshot ID to get. */
    public static String snapshotId = "snapshot-id";

    public static void main(String[] args) {
        SampleCommandLineParser.load(GetPGSnapshot.class, args);
        ...
        SnapshotsTypes.Info info = get(snapshotServiceClient, clusterId, pgId, snapshotId);
        log.info("Got pg snapshot info for snapshot id {}: {}", snapshotId, info);
    }

    public static SnapshotsTypes.Info get(
            SnapshotServiceClient snapshotServiceClient, String clusterId, String pgId, String snapshotId) {
        Snapshots snapshots = snapshotServiceClient.createStub(Snapshots.class);
        return snapshots.get(clusterId, pgId, snapshotId);
    }
}
