...
/**
 * Demonstrates powering off operation of a virtual machine. Works with groups of virtual machines all at one time. You
 * must specify one of --vmname or --datacentername or --host or --all. All virtual machines that match these criteria
 * will have the power off operation issued to them. For example to power off all the visible virtual machines, use the
 * options --all true and all virtual machines will be powered off.
 */
public class PowerOffVm {
    private static final Logger log = LoggerFactory.getLogger(PowerOffVm.class);
    ...
    public static void main(String[] args) throws Exception {
        SampleCommandLineParser.load(PowerOffVm.class, args);
        ...
        if (checkOptions()) {
            ...
            try (VcenterClient client = factory.createClient(username, password, null)) {
                ...
                if (vmMap == null || vmMap.isEmpty()) {
                    ...
                    powerOffVM(vimPort, propertyCollectorHelper, vmMap);
                }
            }
        }
    }
    ...
    private static boolean checkOptions() {
        ...
        if (all) {
            ...
            log.info("Power off operation will be broadcast to ALL virtual machines.");
            run = true;
        } else if (vmName == null
                && datacenterName == null
                && guestId == null
