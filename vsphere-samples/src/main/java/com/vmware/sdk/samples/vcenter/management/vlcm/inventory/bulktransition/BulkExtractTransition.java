...
/**
 * Demonstrates triggering the "extract" and "transition" workflow for a group of clusters or standalone hosts for the
 * bulk transition feature, waiting for the task to complete, and getting the result.
 *
 * <p>Prerequisites:
 *
 * <ul>
 ...
 * </ul>
 */
public class BulkExtractTransition {
    private static final Logger log = LoggerFactory.getLogger(BulkExtractTransition.class);
    ...
    public static void main(String[] args) throws Exception {
        SampleCommandLineParser.load(BulkExtractTransition.class, args);
        ...
        try (VcenterClient client = factory.createClient(username, password, null)) {
            ...
            if (entityType.equalsIgnoreCase("CLUSTER")) {
                ...
                Set<String> clustersToBeTransitioned = new HashSet<>();
                ...
                for (ClustersTypes.ClusterSummary summary : clusterSummaries) {
                    if (summary.getStatus() == ClustersTypes.TransitionStatus.ELIGIBLE) {
                        ...
                        clustersToBeTransitioned.add(summary.getCluster());
                    }
                }
                ...
                if (!clustersToBeTransitioned.isEmpty()) {
                    // Create an entity spec to perform transition
                    entitySpec = new InventoryTypes.EntitySpec.Builder(InventoryTypes.EntitySpec.InventoryType.CLUSTER)
                            .setClusters(clustersToBeTransitioned)
                            .build();
                    InventoryTypes.TransitionSpec.Builder transitionSpec =
                            new InventoryTypes.TransitionSpec.Builder(entitySpec);
                    tasks = client.createStub(Tasks.class);

                    // Invoke transition task
                    taskId = inventoryStub.transition_Task(transitionSpec.build());
                    log.info("Task invoked successfully {}", taskId);
                    ...
                    // Check clusters status using the summary API, after the task is complete
                    getParams = new ClustersTypes.GetParams.Builder(ClustersTypes.GetParams.InventoryType.CLUSTER)
                            .setClusters(clustersToBeTransitioned)
                            .build();
                    clusterInfo = clustersStub.get(getParams, null, null, null);
                    clusterSummaries = clusterInfo.getClusterSummaries();

                    // Clusters with status CONVERTED are transitioned to VLCM, others are not
                    log.info("----------Clusters that are transitioned----------");
                    for (ClustersTypes.ClusterSummary summary : clusterSummaries) {
                        if (summary.getStatus() == ClustersTypes.TransitionStatus.CONVERTED) {
                            log.info("Transitioned Cluster Details ---------- {}", summary);
                        }
                    }
                    log.info("----------Clusters that are not transitioned----------");
                    for (ClustersTypes.ClusterSummary summary : clusterSummaries) {
                        if (summary.getStatus() != ClustersTypes.TransitionStatus.CONVERTED) {
                            log.info("Not Transitioned Cluster Details ---------- {}", summary);
                        }
                    }
                }
                ...
                Set<String> hostsToBeTransitioned = new HashSet<>();
                ...
                for (HostsTypes.HostSummary summary : hostSummaries) {
                    if (summary.getStatus() == HostsTypes.TransitionStatus.ELIGIBLE) {
                        ...
                        hostsToBeTransitioned.add(summary.getHost());
                    }
                }
                ...
                if (!hostsToBeTransitioned.isEmpty()) {

                    // Create an entity spec to perform transition
                    entitySpec = new InventoryTypes.EntitySpec.Builder(InventoryTypes.EntitySpec.InventoryType.HOST)
                            .setHosts(hostsToBeTransitioned)
                            .build();
                    InventoryTypes.TransitionSpec.Builder transitionSpec =
                            new InventoryTypes.TransitionSpec.Builder(entitySpec);
                    tasks = client.createStub(Tasks.class);
                    ...
                    // Check clusters status using the summary API, after the task is complete
                    getParams = new HostsTypes.GetParams.Builder(HostsTypes.GetParams.InventoryType.HOST)
                            .setHosts(hostsToBeTransitioned)
                            .build();
                    hostInfo = hostsStub.get(getParams, null, null, null);
                    hostSummaries = hostInfo.getHostSummaries();

                    // Clusters with status CONVERTED are transitioned to VLCM, others are not
                    log.info("----------Hosts that are transitioned----------");
                    for (HostsTypes.HostSummary summary : hostSummaries) {
                        if (summary.getStatus() == HostsTypes.TransitionStatus.CONVERTED) {
                            log.info("Transitioned Host Details ---------- {}", summary);
                        }
                    }
                    log.info("----------Hosts that are not transitioned----------");
                    for (HostsTypes.HostSummary summary : hostSummaries) {
                        if (summary.getStatus() != HostsTypes.TransitionStatus.CONVERTED) {
                            log.info("Not Transitioned Host Details ---------- {}", summary);
                        }
                    }
                }
            }
